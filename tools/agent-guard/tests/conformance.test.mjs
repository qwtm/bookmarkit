...
describe("agent-guard conformance (ENG-0138)", () => {
  test("the guard implementation is present", () => {
    for (const file of [
      ...
      assert.ok(
        ...
        `${file} must exist — the guard is a governed file, not an optional one`
      );
    }
  });
  ...
  test("Claude Code registers the guard on Bash", () => {
    ...
    assert.ok(
      hookCommands([bash]).some(
        (command) =>
          command.includes("guard-agent-command.mjs") && command.includes("--protocol=claude")
      ),
  ...
  test("Cursor registers the same guard on its own protocol", () => {
    ...
    assert.ok(
      (hooks.hooks?.beforeShellExecution ?? []).some(
        (hook) =>
          (hook.command ?? "").includes("guard-agent-command.mjs") &&
          (hook.command ?? "").includes("--protocol=cursor")
      ),
  ...
  test("Codex registers it too — a guard only one harness honours is not a guard", () => {
    ...
    assert.ok(
      hookCommands(hooks.hooks?.PreToolUse).some(
        (command) =>
          command.includes("guard-agent-command.mjs") && command.includes("--protocol=codex")
      ),
  ...
  test("the worktree-identity hook survives alongside it", () => {
    assert.ok(
      hookCommands(json(".claude/settings.json").hooks?.WorktreeCreate).some((command) =>
        command.includes("claude-worktree-create")
      ),
  ...
  test("the heavy lanes are not pre-approved back open in the permission allow-list", () => {
    ...
    for (const rule of allow) {
      assert.doesNotMatch(
        rule,
        /npm run (ci|test:e2e|test:stories|test:perf|test:cov)/u,
        `${rule} pre-approves a heavy lane that ENG-0138 denies agents by default`
      );
    }
  });
  ...
  test("the guard denies the commands from the incident", () => {
    for (const command of [
      ...
      assert.equal(
        evaluateCommand(command, { env }).allow,
        false,
        `expected the guard to deny: ${command}`
      );
    }
  });
  ...
  test("the guard denies tampering with its own controls", () => {
    for (const command of [
      "AGENT_GUARD_FORCE=1 npm run test:dom",
      "AGENT_GUARD_ASSUME_HUMAN=1 npm run test:dom",
      "node tools/agent-guard/arbiter.mjs grant e2e",
    ]) {
      assert.equal(
        evaluateCommand(command, { env }).allow,
        false,
  ...
  test("ordinary work is untouched", () => {
    for (const command of [
      ...
      assert.equal(
        evaluateCommand(command, { env }).allow,
        true,
        `expected the guard to allow: ${command}`
      );
    }
  });
  ...
  test("ceilings derive from the machine, so no repo can pin an unreachable one", () => {
    ...
    for (const totalMb of [4096, 8192, 16384, 65536]) {
      ...
      assert.ok(
        budget.maxRunMb < totalMb,
        `a ${totalMb} MB machine must cap a run below its own RAM`
      );
      assert.equal(
        clampCeiling(totalMb * 4, budget).ceilingMb,
        budget.maxRunMb,
        "an oversized request must clamp to the cap"
      );
    }
  });
